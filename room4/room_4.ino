...
PubSubClient clientP2(espClient4);
// ----------------------------------------------------------------------------------------------------------------------------------
// DHT Sensor
const int DHTPin = 14;
//các relay
...
void reconnect() {
  ...
  while (!clientP2.connected()) {
    ...
    if (clientP2.connect("ESP8266Client4")) {
...
void loop() 
{
  ...
  if(!clientP2.loop())
  clientP2.connect("ESP8266Client4");
  MQ135.update();
  h = dht.readHumidity();
  h = round(h * 10)/10;
...
// so sánh với giá trị cũ khi có giá trị mới rồi mới gửi.
  ...
  if (now - lastMeasure > 10000) {
    ...
    if(t != last_temp || h !=last_humi || NH3 != last_Nh3 || CO != last_CO || CO2 != last_CO2 || Alcohol !=last_Alcohol){
      ...
      clientP2.publish("/PHONG4", jsonString.c_str());  
    }
  }
  delay(500);
}